...
test("imageStampAspectRatio is width/height and guards against zero height", () => {
  assert.equal(
    imageStampAspectRatio(imageStamp({ widthPx: 200, heightPx: 100 }) as never),
    2,
  );
  assert.equal(
    imageStampAspectRatio(imageStamp({ widthPx: 30, heightPx: 0 }) as never),
    30,
  );
});
...
function worldPointFromLocal(
  ...
  return unrotatePointForAnnotation(
    localPoint,
    worldRect,
    (360 - rotation) % 360,
  );
...
test("resizeImageStampRect keeps the anchor corner fixed in local space when the annotation itself is rotated", () => {
  ...
  assert.ok(
    Math.abs(resultLocal.x2 - localRect.x2) < 1e-6,
    "anchor x did not move",
  );
  assert.ok(
    Math.abs(resultLocal.y1 - localRect.y1) < 1e-6,
    "anchor y did not move",
  );
...
test("resizeFreeTextWidth keeps the anchor edge fixed in local space when the annotation itself is rotated", () => {
  ...
  const resultLocal = rotatedAnnotationRect(resized.rect, rotation);
  assert.ok(
    Math.abs(resultLocal.x1 - localRect.x1) < 1e-6,
    "left edge did not move",
  );
  assert.ok(
    Math.abs(resultLocal.y1 - localRect.y1) < 1e-6,
  ...
  assert.ok(
    Math.abs(resultLocal.y2 - localRect.y2) < 1e-6,
    "top edge did not move",
  );
  assert.ok(
