...
test("annotationBounds and annotationHitTest account for per-annotation rotation", () => {
  ...
  // Inside the rotated (wide, short) footprint but outside the original one.
...
test("image-stamp rotation survives a write -> reimport round trip, pixels included", async () => {
  ...
  for (const rotation of [0, 90, 180, 270] as const) {
    ...
    assert.equal(
      extracted!.rotation,
      rotation,
      `rotation ${rotation}: recovered rotation`,
    );
    ...
    assertRectsClose(
      extracted!.rect,
      rect,
      `rotation ${rotation}: recovered rect`,
    );
    ...
    for (let pixel = 0; pixel < 4; pixel += 1) {
      assert.equal(
        decoded.data[pixel * 4],
        255,
        `rotation ${rotation}: red channel`,
      );
      assert.equal(
        decoded.data[pixel * 4 + 1],
        0,
        `rotation ${rotation}: green channel`,
      );
      assert.equal(
        decoded.data[pixel * 4 + 2],
        0,
        `rotation ${rotation}: blue channel`,
      );
    }
  }
});
...
test("extractStampImage rejects a non-identity /Decode array instead of misreading its pixel bytes", async () => {
  ...
  const mutated = await mutateStampImageDict(
    output,
    0,
    id,
    (imageDict, context) => {
...
async function mutateStampImageDict(
  ...
  const { pdfDoc, imageDict } = await findFormAndImageDicts(
    bytes,
    pageIndex,
    nm,
  );
...
async function mutateFormDict(
  ...
  const { pdfDoc, formDict } = await findFormAndImageDicts(
    bytes,
    pageIndex,
    nm,
  );
