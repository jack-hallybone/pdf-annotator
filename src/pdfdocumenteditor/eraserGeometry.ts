...
function forEachGridCell(
  ...
  const minX = Math.floor(
    (Math.min(bounds.x1, bounds.x2) - padding) / cellSize,
  );
  const maxX = Math.floor(
    (Math.max(bounds.x1, bounds.x2) + padding) / cellSize,
  );
  const minY = Math.floor(
    (Math.min(bounds.y1, bounds.y2) - padding) / cellSize,
  );
  const maxY = Math.floor(
    (Math.max(bounds.y1, bounds.y2) + padding) / cellSize,
  );
