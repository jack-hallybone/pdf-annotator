...
type ImageStampResizeHandleKind =
...
export function resizeImageStampRect(
  ...
  const rotation = annotation.rotation ?? 0;
  // Local un-rotated frame throughout; back to the on-page footprint at the end.
    ...
    case "textHighlight": {
      ...
      return {
        ...
        quadPoints: annotation.quadPoints.map((quad) =>
          quad.map(
            (value, index) => value + (index % 2 === 0 ? delta.x : delta.y),
          ),
        ),
      };
    }
...
export function appearanceRotationMatrix(
  ...
  const abcd =
    ROTATION_APPEARANCE_MATRIX_ABCD[normalized] ??
    ROTATION_APPEARANCE_MATRIX_ABCD[0];
  switch (normalized) {
    case 90:
      return [...abcd, 0, width];
    case 180:
...
export function rotationFromAppearanceMatrix(
  ...
  for (const [rotation, abcd] of Object.entries(
    ROTATION_APPEARANCE_MATRIX_ABCD,
  )) {
    if (
      abcd[0] === roundedA &&
      abcd[1] === roundedB &&
...
export function resampleInkPath(path: PdfPoint[], spacing: number) {
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    distanceSinceLastSample = segmentLength - (distanceAlongSegment - spacing);
    previous = point;
  }
...
function simplifyPathSections(
  ...
  while (pendingSections.length > 0) {
    ...
    pendingSections.push(
      [firstIndex, furthestIndex],
      [furthestIndex, lastIndex],
    );
  }
}

export function isFinitePoint(point: PdfPoint) {
