...
import { viewportPointToPdfPoint, viewportRectToPdfRect } from "./pdfGeometry";
  ...
  for (const span of Array.from(textLayerElement.querySelectorAll("span"))) {
    ...
    if (textNode) {
      for (let index = 0; index < text.length; index += 1) {
        ...
        const clientRects = getTextNodeClientRects(textNode, index, index + 1);
...
export function getTextForHighlights(
  ...
  const highlights = annotations.filter(
    (
      annotation,
    ): annotation is Extract<PdfAnnotation, { kind: "textHighlight" }> =>
...
function textRectOverlapsHighlight(textRect: PdfRect, highlightRect: PdfRect) {
...
function distanceToRect(point: PdfPoint, rect: PdfRect) {
  const x = clamp(
    point.x,
...
function nearestTextRectIndex(textRects: TextLayerRect[], point: PdfPoint) {
  ...
  if (textRects.length > 0 && anchorIndex !== null) {
    ...
    if (nextRects.length > 0) {
      return {
        ...
        coveredText: joinTextLayerSegments(nextTextRects),
      };
    }
  }
