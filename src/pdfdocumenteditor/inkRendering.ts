...
// A stroke thinner than this vanishes at low zoom.
const MIN_INK_LINE_WIDTH = 0.25;
...
export function renderPdfPathCanvas({
  ...
  context.lineWidth = Math.max(MIN_INK_LINE_WIDTH, width);
...
function prepareInkCanvasContext(
  options: Parameters<typeof prepareInkCanvasContextState>[0],
) {
  return prepareInkCanvasContextState(options)?.context ?? null;
}
...
function drawInkAnnotationOnContext(
  ...
  context.lineWidth = Math.max(MIN_INK_LINE_WIDTH, annotation.width * scale);
...
export function drawInkCanvasPath(
  ...
  if (commands.length === 1) {
    ...
    context.beginPath();
    context.arc(x, y, Math.max(MIN_INK_LINE_WIDTH, width / 2), 0, Math.PI * 2);
    context.fill();
    return;
  }
